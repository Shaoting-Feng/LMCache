...
def main():
    ...
    # 第二遍：随机抽样（可重复），保留原索引列
    second = df.sample(n=n, replace=True,
    ...
    combined['occurrence_number'] = combined.groupby(
        'index_in_dataset').cumcount() + 1

    # 输出
    combined.to_csv(args.output_csv, index=False)
    print(
        f"Doubling complete: wrote {len(combined)} rows to {args.output_csv}")
