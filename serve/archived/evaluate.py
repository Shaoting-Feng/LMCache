...
def main():
    ...
    if 'reference_answer' not in df.columns or 'answer' not in df.columns:
        print(
            "Error: CSV file must contain 'reference_answer' and 'answer' columns."
        )
        return

    # Apply the evaluate_answer function to each row and store the result in a new column.
    df['rougeL_fmeasure'] = df.apply(
        lambda row: evaluate_answer(row['answer'], row['reference_answer']),
        axis=1)
    df['f1_score'] = df.apply(
        lambda row: f1_score(row['answer'], row['reference_answer']), axis=1)
