...
for file in csv_files:
    ...
    if len(valid_data) > 1:
        ...
        coeffs = np.polyfit(valid_data['token_count'],
                            valid_data['ttft_numeric'], 1)
        a, b = coeffs  # a is the slope and b is the intercept.
        # Store the coefficients.
        linear_params[file] = {"a": a, "b": b}
        print(
            f"Linear fit for {file}: a (slope) = {a:.2f}, b (intercept) = {b:.2f}"
        )

        poly1d_fn = np.poly1d(coeffs)
        # Generate x values for plotting the regression line.
        x_vals = np.linspace(valid_data['token_count'].min(),
                             valid_data['token_count'].max(), 100)
        plt.plot(x_vals,
                 poly1d_fn(x_vals),
                 color='red',
