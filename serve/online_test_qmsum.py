...
FILES = [
    'dataset/qmsum_processed_bw_17500_18500.csv'
]
...
def execute_openai_request_with_output(row, model: str, client: openai.Client) -> Tuple[float, float, float, float, str]:
    ...
    prompt += f"""
    You are an expert at answering questions and summararizing text based on conversations into concise summaries.  
    Below are example contexts to illustrate the desired format:

    {row.context}

    Now, using the context, please answer the following question.
    **Do not include any heading or the word “Summary:” — just output the answer text.**
