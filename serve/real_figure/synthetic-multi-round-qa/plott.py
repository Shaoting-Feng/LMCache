...
systems = [
    'Prefill', 'GPU Prefix Caching', 'CPU offloading', 'GPU+CPU+SSD (0.5GB/s)'
]
model1_files = [['prefill-3b_output_0.5.csv'], ['gpu-3b_output_0.5.csv'],
                ['cpu-3b_output_0.5.csv'], ['disk-3b_output_0.5.csv']]
model2_files = [['prefill_output_0.5.csv'], ['gpu_output_0.5.csv'],
                ['cpu_output_0.5.csv'], ['disk_output_0.5.csv']]


def create_dummy_csv(filename, ttft_value):
    if not os.path.exists(filename):
        df = pd.DataFrame(
            {'ttft': np.random.normal(ttft_value, ttft_value * 0.05, size=30)})
...
for i, v in enumerate(throughput1):
    ax1b.annotate(f"{v:.1f}", (x[i], throughput1[i]),
                  textcoords="offset points",
                  xytext=(0, 14),
                  ha='center',
                  fontsize=annotate_fontsize,
...
for i, v in enumerate(throughput2):
    ax2b.annotate(f"{v:.1f}", (x[i], throughput2[i]),
                  textcoords="offset points",
                  xytext=(0, 14),
                  ha='center',
                  fontsize=annotate_fontsize,
