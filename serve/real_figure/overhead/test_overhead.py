...
async def run_context(round_idx: int, manager: KVCacheManager,
                      hot_cache: OrderedDict, lock: asyncio.Lock) -> float:
    # Build a single context's chunks
    num_chunks = random.randint(3, 14)
    to_save_list = OrderedDict()
    for chunk_idx in range(num_chunks):
        metadata = CacheManagerMetadata(context_id=[f"context_{round_idx}"],
                                        method=["kivi"],
                                        rate=RATE,
                                        length=CHUNK_BYTES,
                                        ...
                                        score_table=[[(1.0, 1.0),
                                                      ...
                                                      (0.0, 0.2)]],
                                        emerge_id=[1],
                                        disk_score_table=[[(1.0, 0.38),
                                                           (0.7286, 0.36),
                                                           (0.4857, 0.34),
                                                           (0.3714, 0.32),
                                                           (0.0, 0.3)]])
        cache_key = CacheEngineKey(fmt="v1",
    ...
    async with lock:
        for key, upd in update_dict.items():
            ...
            if (update_device == "cpu"
                    and update_rate == 0) or update_device == "disk":
        ...
        for key in to_save_list:
            key.metadata.length = int(key.metadata.length *
                                      decisions.compression_rate)
...
async def schedule_contexts(qps: float, total_rounds: int,
    ...
    for i in range(total_rounds):
        tasks.append(
            asyncio.create_task(run_context(i, manager, hot_cache, lock)))
...
def main():
    parser = argparse.ArgumentParser(
        description=
        "Test overhead of KVCacheManager.inform_new with controlled QPS and concurrency."
    )
