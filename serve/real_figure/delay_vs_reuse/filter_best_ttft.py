...
def find_min_ttft(input_csv, output_csv=None):
    ...
    for w, group in sorted(df.groupby('weight'), key=lambda x: x[0]):
        ...
        if valid.empty:
            print(
                f"Warning: no entries with weighted_rougel ≥ 95% at weight {w}",
                file=sys.stderr)
    ...
    out_df = pd.DataFrame(results,
                          columns=['weight', 'min_ttft_for_rougel_>=95'])
...
def main():
    parser = argparse.ArgumentParser(
        description=
        "For each weight in weighted_averages.csv, find the minimum ttft where weighted_rougel >= 95%."
    )
    ...
    parser.add_argument(
        ...
        '--output',
        help=
        "Optional path to write results (CSV). If omitted, prints to stdout.")
