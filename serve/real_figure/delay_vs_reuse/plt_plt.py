...
def plot_selected_weights(csv_path, output_path=None):
    ...
    mask = df['weight'].apply(
        lambda x: any(abs(x - w) < tol for w in target_weights))
    subset = df[mask].sort_values('weight')

    if len(subset) < len(target_weights):
        missing = set(target_weights) - set(
            round(w, 4) for w in subset['weight'])
    ...
    # 绘制 “Ours” 曲线
    ax.plot(x, y_ours, marker='o', markersize=10, linewidth=5, label='Ours')

    # 绘制 “Prefill” 平线
    ax.plot(x, y_prefill, linestyle='--', linewidth=3, label='Prefill')
...
if __name__ == '__main__':
    ...
    parser.add_argument('csv_path', help='输入 CSV 文件路径，例如 /mnt/data/result.csv')
