...
from lmcache.experimental.storage_backend.naive_serde.new_pack import (
    triton_quantize_and_pack_along_last_dim, quant_and_pack_kcache,
...
def unpack_tensor(v_code: torch.FloatTensor, bits: int, pack_dim: int):
    ...
    new_shape = shape[:pack_dim] + (shape[pack_dim] *
                                    feat_per_int, ) + shape[pack_dim + 1:]
    unpacked_v_code = torch.zeros(new_shape,
                                  dtype=torch.int8,
                                  device=v_code.device)
    ...
    if pack_dim == 2:
        unpacked_v_code = (
            (v_code[packed_indices] >>
             (j * bits)[None, None, :, None]).to(torch.int16)) & num
    elif pack_dim == 3:
        unpacked_v_code = ((v_code[packed_indices] >>
                            (j * bits)).to(torch.int16)) & num
    else:
...
class KIVISerializer(Serializer):
    ...
    def preprocess_compressed_dict(self,
                                   compressed_dict,
                                   quant_common_dtype=torch.float32):
        """
        ...
        all_keys = [
            "k_min", "v_min", "k_scale", "v_scale", "residual_k", "residual_v"
        ]
        ...
        for d in compressed_dict:
            ...
            entry_flat = torch.cat(parts) if parts else torch.tensor(
        ...
        big_tensor = torch.cat(
            non_quant_flattened_entries
        ) if non_quant_flattened_entries else torch.tensor(
            [], dtype=torch.float32, device="cuda")
        ...
        for d in compressed_dict:
            ...
            meta['entry_length'] = offset
            entry_flat = torch.cat(parts) if parts else torch.tensor(
                [], dtype=quant_common_dtype, device="cuda")
        ...
        for entry in quant_flattened_entries:
            ...
            cumulative = end
        quant_big_tensor = torch.cat(
            quant_flattened_entries
        ) if quant_flattened_entries else torch.tensor(
            [], dtype=quant_common_dtype, device="cuda")
    ...
    def serialize(self, memory_obj: MemoryObj, bits: int = 4) -> MemoryObj:
        ...
        for layer in range(t.shape[1]):
            k = t[0][layer].reshape((t[0][layer].shape[0], self.num_heads,
                                     self.head_size)).permute(
                                         (1, 0, 2)).unsqueeze(0)
            v = t[1][layer].reshape((t[0][layer].shape[0], self.num_heads,
                                     self.head_size)).permute(
                                         (1, 0, 2)).unsqueeze(0)
            ...
            if rounded_len > 0:
                ...
                quant_k_triton, k_scale_triton, k_min_triton = triton_quantize_and_pack_along_last_dim(
                    rounded_k.transpose(2, 3).contiguous(), self.group_size,
                    bits)
                quant_v_triton, v_scale_triton, v_min_triton = triton_quantize_and_pack_along_last_dim(
                    rounded_v.contiguous(), self.group_size, bits)
                ...
                k_scale = k_scale_triton.permute(0, 1, 3, 2).reshape(
                    (k_scale_triton.shape[0], k_scale_triton.shape[1],
                     k_scale_triton.shape[-1], 1, k_scale_triton.shape[2]))
                k_min = k_min_triton.permute(0, 1, 3, 2).reshape(
                    (k_min_triton.shape[0], k_min_triton.shape[1],
                     k_min_triton.shape[-1], 1, k_min_triton.shape[2]))
                quant_v = quant_v_triton
                v_scale = v_scale_triton.reshape(
                    (v_scale_triton.shape[0], v_scale_triton.shape[1],
                     v_scale_triton.shape[2], v_scale_triton.shape[3], 1))
                v_min = v_min_triton.reshape(
                    (v_scale_triton.shape[0], v_scale_triton.shape[1],
                     v_scale_triton.shape[2], v_scale_triton.shape[3], 1))
        ...
        big_tensor, metadata, entry_offsets, quant_big_tensor, quant_metadata, quant_entry_offsets = self.preprocess_compressed_dict(
            compressed_dict)
        ...
        allocated_obj = self.memory_allocator.allocate(
            saved_tensor.shape, saved_tensor.dtype, fmt=MemoryFormat.KV_BLOB2)
...
class KIVIDeserializer(Deserializer):
    ...
    def resume_non_quant_entry(self, big_tensor, meta, entry_offset):
        ...
        for key in meta['keys_order']:
            ...
            if key_meta['numel'] == 0:
                ...
                off = key_meta['offset']
                resumed[key] = entry_tensor[off:off + key_meta['numel']].view(
                    key_meta['shape'])
        return resumed
    ...
    def deserialize(self,
                    memory_obj: Union[MemoryObj, str],
                    bits: int = 4,
                    metadata=[],
                    entry_offsets=[],
                    split_metadata=None,
                    quant_metadata=[],
                    quant_entry_offsets=[]) -> MemoryObj:
        ...
        big_tensor_bytes_rec = memory_obj.tensor[:split_metadata[
            "num_big_bytes"]]
        quant_tensor_bytes_rec = memory_obj.tensor[
            split_metadata["num_big_bytes"]:]

        if type(memory_obj) == TensorMemoryObj:
            self.memory_allocator.ref_count_down(memory_obj)

        big_tensor_rec = big_tensor_bytes_rec.view(
            split_metadata["big_tensor_dtype"])
        quant_tensor_rec = quant_tensor_bytes_rec.view(
            split_metadata["quant_tensor_dtype"])
        ...
        for layer in range(len(metadata)):
            ...
            non_quant_entry = self.resume_non_quant_entry(
                non_quant_gpu_tensor, metadata[layer], entry_offsets[layer])
            # Resume quant fields.
            quant_entry = self.resume_quant_entry(quant_gpu_tensor,
                                                  quant_metadata[layer],
                                                  quant_entry_offsets[layer])
            ...
            if quant_k is not None:
                dequant_k = unpack_and_dequant_kcache(quant_k, k_scale, k_min,
                                                      self.group_size, bits)
                dequant_v = unpack_and_dequant_vcache(quant_v, v_scale, v_min,
                                                      self.group_size, bits)
        ...
        kv_chunk = blob.reshape(
            *blob.shape[:-2],
            hidden_dim)  # [nlayers, 2, ntokens, num_heads, head_size]
